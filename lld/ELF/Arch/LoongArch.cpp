...
// The LoongArch `pcalau12i` (PC-aligned add upper 12 bits immediate) is
// behaviorally equivalent to the AArch64 `adrp`, so a similar concept of
// "page" also applies here.
static uint64_t getLoongArchPage(uint64_t p) {
  return p & ~static_cast<uint64_t>(0xFFF);
}

// Calculate the page offset between dest and PC.
...
//     ldx.d     A, A, T
//
// in which case the "pc + hi20" part is separate from the rest (including
// the higher 32-bit half and lo12), so the higher 32 bits need a nudge too.
